...
static void ArgumentList_OutputError(int Code, const char *pOptionText, ERROR_FUNCTION pErrorFunction)
{
	switch (Code)
	{
		...
		{
			pErrorFunction("Option \"%s\" is not handled!",pOptionText);
		}
		break;

		case OPTION_NOT_RECOGNISED:
		{
			pErrorFunction("Option \"%s\" is not recognised!",pOptionText);
		}
		break;

		case OPTION_MISSING_PARAMETER:
		{
			pErrorFunction("Option \"%s\" is missing parameter(s)!",pOptionText);
		}
		break;
	}
}
